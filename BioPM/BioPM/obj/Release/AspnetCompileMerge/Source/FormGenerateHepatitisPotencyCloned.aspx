<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="FormGenerateHepatitisPotencyCloned.aspx.cs" Inherits="BioPM.FormGenerateHepatitisPotencyCloned" %>
...
<script runat="server">
    ...
    protected void GenerateRandomNumber(List<int> setnumber)
    {
        ...
        {
            BioPM.ClassObjects.QualityControlCatalog.InsertNumberTest(setnumber[i], rand.Next(1, setnumber.Count));
        }
    }
    ...
    protected void GetRandomAnimalDate()
    {
        ...
        foreach (object[] data in BioPM.ClassObjects.QualityControlCatalog.GetAnimalRandomDateByType("16"))
        {
    ...
    protected void InsertDilutionTestIntoDatabase()
    {
        ...
        while (index < (((dilution * animalperdilution) / animalpercage) + 2))
        {
            if (malecagenumbers.Count > 0)
            {
                if (control < 2)
                {
                    BioPM.ClassObjects.QualityControlCatalog.InsertQCDilutionTestNegatifControl(txtRandomDate.Text, ddlRandomDate.SelectedValue, QCTID, "16", itemid, malecagenumbers[0].ToString(), dilutionnumber.ToString(), samplenumber.ToString(), Session["username"].ToString());
                    control++;
                }
                else
                {
                    BioPM.ClassObjects.QualityControlCatalog.InsertQCDilutionTest(txtRandomDate.Text, ddlRandomDate.SelectedValue, QCTID, "16", itemid, malecagenumbers[0].ToString(), dilutionnumber.ToString(), samplenumber.ToString(), Session["username"].ToString());
            ...
            if (femalecagenumbers.Count > 0)
            {
                if (control < 2)
                {
                    BioPM.ClassObjects.QualityControlCatalog.InsertQCDilutionTestNegatifControl(txtRandomDate.Text, ddlRandomDate.SelectedValue, QCTID, "16", itemid, femalecagenumbers[0].ToString(), dilutionnumber.ToString(), samplenumber.ToString(), Session["username"].ToString());
                    control++;
                }
                else
                {
                    BioPM.ClassObjects.QualityControlCatalog.InsertQCDilutionTest(txtRandomDate.Text, ddlRandomDate.SelectedValue, QCTID, "16", itemid, femalecagenumbers[0].ToString(), dilutionnumber.ToString(), samplenumber.ToString(), Session["username"].ToString());
                    if ((index + 1) % dilutionnumberdivided == 0) dilutionnumber++;
                    if ((index + 1) % samplenumberdivided == 0) samplenumber++;
                    index++;
                }
            ...
            if (malecagenumbers.Count == 0 && femalecagenumbers.Count == 0) break;
        }
    }
    ...
    protected void RunRandomToGetDilutionSample()
    {
        ...
        else if (!BioPM.ClassEngines.ValidationFactory.ValidateToCompareValue(((Convert.ToInt16(txtAnimalPerDilution.Text) / Convert.ToInt16(txtAnimalPerCage.Text)) * Convert.ToInt16(txtDilutionNumber.Text))+2, Convert.ToInt16(txtAnimalNumber.Text) / Convert.ToInt16(txtAnimalPerCage.Text)))
        {
        ...
        {
            GenerateRandomNumber(BioPM.ClassObjects.QualityControlCatalog.GetCageNumberByID(ddlRandomDate.SelectedValue));
            ...
            <div class="col-sm-12">
                <section class="panel">
                    <header class="panel-heading">
                        HEPATITIS POTENCY RANDOM SAMPLE ENTRY FORM
