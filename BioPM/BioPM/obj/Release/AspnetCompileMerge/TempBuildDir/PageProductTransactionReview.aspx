<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="PageProductTransactionReview.aspx.cs" Inherits="BioPM.PageProductTransactionReview" %>
...
<script runat="server">
    ...
    protected void GenerateDataProduction()
    {
        ...
        foreach (object[] data in BioPM.ClassObjects.ProductionCatalog.GetProductProductionByBatch(txtBatch.Text))
        {
            ...
            dt.Rows.Add(new object[] { index.ToString(), data[2].ToString(), BioPM.ClassEngines.DateFormatFactory.GetDateFormat(data[0].ToString()), "false" });
            index++;
            _ProductionData = dt;
        }
        this.GridViewProduction.DataSource = _ProductionData;
    ...
    protected void GenerateDataFormula()
    {
        ...
        foreach (object[] data in BioPM.ClassObjects.ProductionCatalog.GetProductFormulaByBatch(txtBatch.Text))
        {
            System.Data.DataTable dt = _FormulaData;
            int newid = dt.Rows.Count + 1;
            dt.Rows.Add(new object[] { index.ToString(), data[3].ToString(), data[4].ToString(), Convert.ToDecimal(data[5].ToString()).ToString("F"), data[6].ToString(), "false" });
            index++;
            _FormulaData = dt;
        }
        this.GridViewFormula.DataSource = _FormulaData;
