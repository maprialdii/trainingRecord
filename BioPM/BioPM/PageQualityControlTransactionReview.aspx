...
<script runat="server">
    ...
    protected void UpdateDataQualityControlOnDatabase()
    {
        bool isReviewed = true;

        for (int i = 0; i < _QCData.Rows.Count; i++)
        {
            BioPM.ClassObjects.ProductionCatalog.UpdateProductionQualityControlResultReview(txtBatch.Text, _QCData.Rows[i]["QCTYP"].ToString(), _QCData.Rows[i]["QCSTY"].ToString(), _QCData.Rows[i]["QCRES"].ToString(), _QCData.Rows[i]["UNTID"].ToString(), ((CheckBox)GridViewQC.Rows[i].FindControl("QCVALReview")).Checked == false ? "0" : "1", Session["username"].ToString());
        ...
        if (isReviewed) InsertProductionQCTransactionFlow("REVIEW");
        else InsertProductionQCTransactionFlow("CORECTION");

    }

    protected void InsertProductionQCTransactionFlow(string FLWST)
    {
        BioPM.ClassObjects.ProductionCatalog.InsertProductionTransactionFlow(txtBatch.Text, "2", FLWST, Session["username"].ToString().ToString(), txtNote.Text, Session["username"].ToString());
    }
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        {
            UpdateDataQualityControlOnDatabase();
            Response.Redirect("PageBatchProductQualityControlReview.aspx");
        }
        else
        {
