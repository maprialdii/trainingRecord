...
<script runat="server">
    ...
    {
        if (BioPM.ClassObjects.ProductionCatalog.GetProductionStatusByBatch(BATCH, "1") == null)
        {
            return "<span class='label label-default'> PENDING </span>";
        }
        else
        {
            switch (BioPM.ClassObjects.ProductionCatalog.GetProductionStatusByBatch(BATCH, "1")[2].ToString().ToLower())
            {
                case "quarantine":
                    {
    ...
    protected string GetPageAction(string BATCH)
    {
        ...
        else
        {
            switch (BioPM.ClassObjects.ProductionCatalog.GetProductionStatusByBatch(BATCH, "2")[2].ToString().ToLower())
            {
                case "quarantine":
                    {
                        return "<td align='center'><a class='edit' href='PageProductionTransaction.aspx?key=" + BATCH + "'> UPDATE </a></td>";
                    }
                case "reviewe":
                    {
