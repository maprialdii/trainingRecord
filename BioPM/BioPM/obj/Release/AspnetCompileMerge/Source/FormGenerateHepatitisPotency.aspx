<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="FormGenerateHepatitisPotency.aspx.cs" Inherits="BioPM.FormGenerateHepatitisPotency" %>
...
<script runat="server">
    ...
    protected void InsertDilutionTestIntoDatabase()
    {
        ...
        List<int> numbers = new List<int>(BioPM.ClassObjects.QualityControlCatalog.GetNumberTest());
        ...
        for (int i = 0; i < ((dilution * animalperdilution) / animalpercage) + 2; i++)
        {
            if (i < 2)
            {
                BioPM.ClassObjects.QualityControlCatalog.InsertQCDilutionTestNegatifControl(txtRandomDate.Text, ddlRandomDate.SelectedValue, QCTID, "16", itemid, numbers[i].ToString(), dilutionnumber.ToString(), samplenumber.ToString(), Session["username"].ToString());
            }
            else
            {
                BioPM.ClassObjects.QualityControlCatalog.InsertQCDilutionTest(txtRandomDate.Text, ddlRandomDate.SelectedValue, QCTID, "16", itemid, numbers[i].ToString(), dilutionnumber.ToString(), samplenumber.ToString(), Session["username"].ToString());
                if (((i + 1)-2) % dilutionnumberdivided == 0) dilutionnumber++;
                if (((i + 1)-2) % samplenumberdivided == 0) samplenumber++;
            }
        }
    }
