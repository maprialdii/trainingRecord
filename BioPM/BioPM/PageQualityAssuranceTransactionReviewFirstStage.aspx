<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="PageQualityAssuranceTransactionReviewFirstStage.aspx.cs" Inherits="BioPM.PageQualityAssuranceTransactionReviewFirstStage" %>
...
<script runat="server">
    ...
    protected bool CheckReviewStatus()
    {
        bool isReviewed = true;

        for (int i = 0; i < _QCData.Rows.Count; i++)
        {
            if (((CheckBox)GridViewQC.Rows[i].FindControl("QCVALReview")).Checked == false)
            {
                isReviewed = false;
            }
        }
        ...
        if (isReviewed)
        {
            ...
            {
                if (((CheckBox)GridViewQC.Rows[i].FindControl("FRVALReview")).Checked == false)
                {
                    isReviewed = false;
                }
            }
    ...
    protected void InsertProductionQCTransactionFlow(string FLWST)
    {
        BioPM.ClassObjects.ProductionCatalog.InsertProductionTransactionFlow(txtBatch.Text, "3", FLWST, Session["username"].ToString().ToString(), txtNote.Text, Session["username"].ToString());
    }
    ...
    protected void InsertProductionTransactionFlow()
    {
        ...
        {
            if (txtProductStatus.Text == "QA QUARANTINE")
                BioPM.ClassObjects.ProductionCatalog.InsertProductionTransactionFlow(txtBatch.Text, "3", ddlApprovalStatus.SelectedValue, _ReviewerData.Rows[i]["PERNR"].ToString(), "", Session["username"].ToString());
        }
    }
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        {
            if (ddlApprovalStatus.SelectedValue == "QA REVIEW" && !CheckReviewStatus())
            {
                ClientScript.RegisterStartupScript(this.GetType(), "myalert", "alert('" + "TO RELEASE PRODUCT YOU HAVE TO CHECK ALL COMPONENTS!" + "');", true);
            }
            else
            {
            ...
            Response.Redirect("PageBatchProductQualityAssurance.aspx");
        }
        else
        {
                                ...
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <asp:DropDownList ID="ddlApprovalStatus" runat="server" class="form-control m-bot15">
                                            <asp:ListItem Value="QA REVIEW">Release</asp:ListItem> 
                                            <asp:ListItem Value="QA CORECTION">Reject</asp:ListItem>
                                        </asp:DropDownList>
